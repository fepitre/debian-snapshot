...
# DATABASE_URI = 'sqlite:////snapshot_db/snapshot.db'
...
FilesTimestamps = Table(
    'files_timestamps', Base.metadata,
    Column('file_sha256', String, ForeignKey('files.sha256')),
    Column('timestamp_value', String, ForeignKey('timestamps.value'))
)
...
class DBfile(Base):
    ...
    timestamps = relationship("DBtimestamp", secondary=FilesTimestamps)
...
class DBbinpkg(Base):
    ...
    name = Column(String, primary_key=True)
    version = Column(String, primary_key=True)
    # files = relationship("DBfile", secondary=BinpkgFiles)
