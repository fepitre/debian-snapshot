    ...
    def provision_database(self, archive, timestamp, suites, components, arches, ignore_provisioned=False):
        ...
        try:
            ...
            if to_add_files:
                ...
                stmt_insert_new_first_location_to_file = """
                INSERT INTO first_files_locations (file_sha256, archive_name, timestamp_value, suite_name, component_name)
                SELECT t.sha256, t.archive_name, min(t.timestamp_value) first_time, t.suite_name, t.component_name
                FROM tempfiles t GROUP BY t.sha256, t.archive_name, t.suite_name, t.component_name
                ON CONFLICT (file_sha256, archive_name, suite_name, component_name) DO UPDATE
                SET timestamp_value = EXCLUDED.timestamp_value WHERE EXCLUDED.timestamp_value < first_files_locations.timestamp_value
                """
                session.execute(stmt_insert_new_first_location_to_file)
                session.commit()

                stmt_insert_new_last_location_to_file = """
                INSERT INTO last_files_locations (file_sha256, archive_name, timestamp_value, suite_name, component_name)
                SELECT t.sha256, t.archive_name, max(t.timestamp_value) first_time, t.suite_name, t.component_name
                FROM tempfiles t GROUP BY t.sha256, t.archive_name, t.suite_name, t.component_name
                ON CONFLICT (file_sha256, archive_name, suite_name, component_name) DO UPDATE
                SET timestamp_value = EXCLUDED.timestamp_value WHERE EXCLUDED.timestamp_value > last_files_locations.timestamp_value
                """
                session.execute(stmt_insert_new_last_location_to_file)
                session.commit()
